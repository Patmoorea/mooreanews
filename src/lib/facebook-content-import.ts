...
> {
  ...
  if (
    config.importAllFeedPosts &&
    !cover &&
    isFacebookAutoGeneratedPlaceholder({ title, excerpt, body })
  ) {
    return { ok: false, reason: "empty_shell" };
  }
...
async function repairPendingPlaceholderFacebookArticlesFromDb(
  ...
  for (const row of data ?? []) {
    ...
    const stillPlaceholder =
      ...
      isFacebookAutoGeneratedPlaceholder({
        title: articleTitleFromPost(
          "",
          config,
          false,
          freshness.publishedAt,
        ),
        excerpt: `Publication Facebook ${config.pageName}`,
        body: buildBody(
          "",
          permalinkForPost(post, config.graphPageId ?? "350029589936"),
          config.pageName,
        ),
      });
