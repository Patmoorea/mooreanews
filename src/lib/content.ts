...
export async function getArticleBySlug(
  ...
  if (db) {
    ...
    if (shouldHideEmptyFacebookShell(article)) return undefined;
    if (
...
/** Article public : 404 coquilles vides, 301 vers la fiche dédupliquée si doublon FB. */
export async function resolvePublicArticle(
  ...
  if (db) {
    const article = articleFromRow(db);
    if (shouldHideEmptyFacebookShell(article)) {
    ...
    const winners = await getArticles();
    const key = articleDisplayDedupeKey(article);
    const winner = winners.find((w) => articleDisplayDedupeKey(w) === key);
    if (winner && winner.slug !== slug) {
      return { status: "redirect", slug: winner.slug };
    }
    return { status: "ok", article };
  }
  const fallback = (articlesData as Article[]).find((a) => a.slug === slug);
  if (fallback) return { status: "ok", article: fallback };
  return { status: "missing" };
}
