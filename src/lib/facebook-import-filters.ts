...
/** Article visible sur le site (texte seul, affiche seule, ou les deux). */
export function isFacebookArticleCompleteOnSite(row: {
  ...
  if (isFacebookAutoGeneratedPlaceholder(row)) {
    const cover = row.cover_url?.trim() ?? "";
    const hasPersistedCover =
      Boolean(cover) &&
      !cover.includes("fbcdn.net") &&
      !cover.includes("fbsbx.com");
    if (hasPersistedCover && / — affiche · /i.test(row.title.trim())) {
      return true;
    }
    return false;
  }
  ...
  if (core.length >= 15 && !isFacebookJunkText(core)) return true;
  return false;
}
