...
type Props = {
  ...
  /** Encart pub (composant serveur passé par le parent) */
  inlineAd?: ReactNode;
};

export function ArticlesFilterClient({
  articles,
  showInlineAds = false,
  inlineAd,
}: Props) {
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((a, index) => (
            <Fragment key={a.slug}>
              {showInlineAds && inlineAd && index > 0 && index % 6 === 0 ? (
                <div className="sm:col-span-2 lg:col-span-3">{inlineAd}</div>
              ) : null}
              <PublicationCard
